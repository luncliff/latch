...
#if !defined(__ANDROID_API__) || (defined(__ANDROID_API__) && __ANDROID_API__ >= 28)

    if (auto ec = pthread_mutexattr_setprotocol(&attr, PTHREAD_PRIO_NONE))
        throw system_error{ec, system_category(), "pthread_mutexattr_setprotocol"};
#endif
}
...
CheckCurrentCount:
        ...
        return reason;
    {
        if (auto ec = pthread_mutex_lock(addressof(this->mtx)))
            return ec;
        reason = pthread_cond_timedwait(addressof(this->cv), addressof(this->mtx), addressof(until));
